...
void init_simple_htS(struct HashtableS *ht, int hmax, unsigned int (*hash_function)(void*), int (*compare_function)(void*, void*)) {
    /* TODO */
 ...
 */
void put_simpleS(struct HashtableS *ht, void *key, size_t key_size_bytes, void *value) {
    /* TODO */
...
void* get_simpleS(struct HashtableS *ht, void *key) {
    /* TODO */
    int index = ht->hash_function(key) % ht->hmax;
    for(struct Node *it = ht->buckets[index].head; it != NULL; it = it->next) {
        if(!ht->compare_function(((struct info*)it->data)->key, key)) {
 ...
 * 0, altfel.
 */
int has_key_simpleS(struct HashtableS *ht, void *key) {
    /* TODO */
    int index = ht->hash_function(key) % ht->hmax;
    for(struct Node *it = ht->buckets[index].head; it != NULL; it = it->next) {
        if(!ht->compare_function(((struct info*)it->data)->key, key)) {
 ...
 */
void free_simple_htS(struct HashtableS *ht) {
    /* TODO */
    struct Node *it;
